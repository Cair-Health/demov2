...
const handlePaperPlaneClick = async () => {
  ...
  console.log(sessionID_policies)
  ...
  try {
    ...
    const getResponseStatus_policies = {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    if(selectedDocType === "Policies"){
      ...
      const checkStatus = async () => {
        const statusResponse = await fetch("https://chat.cairhealth.com/get_status_policies/", getResponseStatus_policies);
        const statusData = await statusResponse.json();
        const status = statusData.status;
        console.log("Status:", statusResponse);
    if (status === "Completed.") {
      // Progress tracking complete, do something
      console.log("Progress tracking complete");
    } else {
      // Progress tracking still ongoing, continue checking
      setTimeout(checkStatus, 10); // Check status again after 5 seconds
    }
      }
    ...
    if (!getResponseResponse) {
      throw new Error('Failed to start chat: Response object is undefined');
    }
